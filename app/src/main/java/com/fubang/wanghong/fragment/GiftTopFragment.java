...
import com.fubang.wanghong.adapters.GiftTopAdapter;
...
public class GiftTopFragment extends BaseFragment
        implements PullToRefreshBase.OnRefreshListener, Callback<GiftTopListEntity>
{
...
//        Log.d("123",getArguments().getString(AppConstant.HOME_TYPE));
                        ...
                        return new Converter<ResponseBody, GiftTopListEntity>() {
                            ...
                            public GiftTopListEntity convert(ResponseBody value) throws IOException {
                                ...
                                try {
                                    ...
                                    List<GiftTopEntity> list = new ArrayList<>();
                                    JSONObject obj = new JSONObject(value.string());
                                    JSONArray array = obj.getJSONArray("topArr");
                                    for (int i = 0; i < array.length(); i++) {
                                        JSONObject object = array.getJSONObject(i);
                                        GiftTopEntity giftTopEntity = new GiftTopEntity();
                                        giftTopEntity.setNbuddyid(object.getString("nbuddyid"));
                                        giftTopEntity.setCalias(object.getString("calias"));
                                        giftTopEntity.setCphoto(object.getString("cphoto"));
                                        giftTopEntity.setNcount(object.getString("ncount"));
                                        list.add(giftTopEntity);
                                    }
                                    entity.setTopArr(list);
    ...
    public void onResponse(Call<GiftTopListEntity> call, Response<GiftTopListEntity> response) {
        ...
        call.cancel();
        data.clear();
        List<GiftTopEntity> list = response.body().getTopArr();
        if (list != null) {
