...
//        });
        ...
        followImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        roomMain.getRoom().getChannel().followRoom(roomId,Integer.parseInt(StartUtil.getUserId(TestActivity.this)));
                    }
                }).start();
        ...
        fullImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                finish();
                startActivity(RoomLandActivity_.intent(TestActivity.this).extra("roomIp",roomIp).extra("roomId",roomId+"").extra("roomPwd",roomPwd+"").get());
            }
        });
//        roomControl.setOnTouchListener(this);
        ...
        gameBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String pwd = StartUtil.getUserPwd(TestActivity.this);
                String mdPwd = stringToMD5(pwd);
                String userId = StartUtil.getUserId(TestActivity.this);
                String mac = StartUtil.getDeviceId(TestActivity.this);
...
//                        if (userSendBtn.getText().toString().contains("大厅")) {
                        ...
                        if (chatToFlag == 0) {
                            new Thread(new Runnable() {
                                @Override
                                public void run() {
                                    roomMain.getRoom().getChannel().sendChatMsg(0, (byte) 0, (byte) 0, "<FONT style=\"FONT-FAMILY:宋体;FONT-SIZE:14px; COLOR:#000000\">" + msgText + "</FONT>", StartUtil.getUserName(TestActivity.this), Integer.parseInt(StartUtil.getUserLevel(TestActivity.this)));
//                                    editText.setText("");
                                }
                            }).start();
...
//                        } else if (!TextUtils.isEmpty(sendToUser.getUseralias())) {
                            ...
                            new Thread(new Runnable() {
                                @Override
                                public void run() {
                                    roomMain.getRoom().getChannel().sendChatMsg(sendToUser.getUserid(), (byte) 0, (byte) 1, "<FONT style=\"FONT-FAMILY:宋体;FONT-SIZE:14px; COLOR:#000000\">" + msgText + "</FONT>", StartUtil.getUserName(TestActivity.this),Integer.parseInt(StartUtil.getUserLevel(TestActivity.this)));
                                    editText.setText("");
                                }
                            }).start();
                        } else {
...
//            public void run() {
        if (err == 503){
            ...
            tv_go.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            play.start();
                            roomMain.Start(roomId,Integer.parseInt(StartUtil.getUserId(TestActivity.this)), StartUtil.getUserPwd(TestActivity.this), ip, port, pwd);
                        }
                    }).start();
                }
            });
            ...
            Toast.makeText(TestActivity.this, "您已被封号请联系客服", Toast.LENGTH_SHORT).show();
            finish();
        }else if(err == 417){
            Toast.makeText(TestActivity.this, "该房间限制等级进入", Toast.LENGTH_SHORT).show();
        }
//            }
//        });
        ...
        if (mgr != null){
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    tmp.StopRTPSession();
                    tmp.Uninit();
                    roomMain.getRoom().getChannel().sendLeaveRoom(Integer.parseInt(StartUtil.getUserId(TestActivity.this)));
...
//        Log.d("123","onDestory---");
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        if(event.getAction() == MotionEvent.ACTION_UP) {
            ...
            y2 = event.getY();
            if(y1 - y2 > 50) {
//                Toast.makeText(TestActivity.this, "向上滑", Toast.LENGTH_SHORT).show();
            } else if(y2 - y1 > 50) {
//                Toast.makeText(TestActivity.this, "向下滑", Toast.LENGTH_SHORT).show();
            } else if(x1 - x2 > 50) {
                if (micUsers.size() != 1) {
/**
 *   if (micUsers.size()>1) {
...
//                Toast.makeText(TestActivity.this, "向左滑", Toast.LENGTH_SHORT).show();
            } else if(x2 - x1 > 50) {
                if (micFlag == 1){
                    micFlag--;
//                    textBackImage.setVisibility(View.VISIBLE);
